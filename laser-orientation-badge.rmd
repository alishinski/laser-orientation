...
-   Why is there a bunch of stuff missing in the q variables?

-   What are the last 3 variables about?
...
-   Less rows when I increase the cutoff
...
```{r}
ggplot(d, aes(x = section, y = FinalGradeCEMS)) +
  geom_point()
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject + section, data = d)
summary(m2)
```
